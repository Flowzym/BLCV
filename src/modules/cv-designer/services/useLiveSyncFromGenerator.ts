...
export function useLiveSyncFromGenerator(debounceMs = 200) {
  ...
  // robuste Dependency – triggert auch bei In-Place-Änderungen
  const sig = useMemo(() => {
    const slim = {
      pd: ll?.personalData ?? {},
      erf: ll?.berufserfahrung ?? ll?.workExperience ?? ll?.experience ?? [],
      edu: ll?.ausbildung ?? ll?.education ?? [],
      m: margins,
    };
    try {
    ...
    timer.current = window.setTimeout(() => {
      ...
      for (const m of mapped) {
        ...
        if (m.group === "kontakt" && !contactPlaced) {
          ...
          const frame = computeFrameForRow(
            "right",
            0,
            margins,
            tpl.baseSize.width,
          ...
          const meta = {
            source: { key: m.sourceKey, group: m.group, template: tpl.id },
          };
      ...
      if (!elements.length && adds.length) {
        ...
        for (const a of adds) {
          const sec = buildSectionFromTemplate(
            a.tpl,
            a.frame,
            a.texts,
            a.meta,
  ...
  }, [sig]); // <— EIN Signatur-String statt zig lockerer Refs
}
