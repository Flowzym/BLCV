...
export default function FabricCanvas(){
  ...
  // key-space:
  //   Container:  sec:<sectionId>
  //   Part:       part:<sectionId>:<partKey>
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(!(c as any).__bl_eventsBound){
        ...
        c.on("selection:created",()=>{ if(syncingRef.current) return; select(selectionIds()); });
        c.on("selection:updated",()=>{ if(syncingRef.current) return; select(selectionIds()); });
        c.on("selection:cleared",()=>{ if(syncingRef.current) return; select([]); });

        // Snap + Parts nicht verschiebbar
        c.on("object:moving",(e:any)=>{
          const o=e.target; if(!o||o.selectable===false) return;
          if (o.__bl_part) { o.set({ left:o.left, top:o.top }); return; }
          o.set({
        ...
        // Container-Scaling; Parts bleiben relativ
        ...
        // Löschen: nur Container erlauben
    ...
    return ()=>{
      ...
      try { const act:any = c?.getActiveObject?.(); if(act?.exitEditing) act.exitEditing(); } catch {}
      try { c?.dispose?.(); } catch {}
...
function applyTextStyle(
  ...
  obj.set({
    ...
    fill: color, fontWeight, fontStyle: italic ? "italic" : "normal",
...
function reconcileCanvas(
  ...
  for(const el of elements){
    // Foto (einfaches Rechteck)
    if (el.kind==="photo"){
      const key=`sec:${el.id}`;
      let obj=map.get(key);
      if(!obj){
        obj = new fabric.Rect({
          left: el.frame.x, top: el.frame.y, width: el.frame.width, height: el.frame.height,
    ...
    if(!container){
      container = new fabric.Rect({
        left: el.frame.x, top: el.frame.y, width: el.frame.width, height: el.frame.height,
        fill:"rgba(0,0,0,0)", stroke:"#e5e7eb", strokeUniform:true, selectable:true
      });
    ...
    for(const part of (el as any).parts){
      ...
      if (!pObj){
        ...
        // Parts sind nicht verschiebbar
        (pObj as any).lockMovementX = true;
        (pObj as any).lockMovementY = true;
