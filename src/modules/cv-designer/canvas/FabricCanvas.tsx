...
export default function FabricCanvas(){
  ...
  useEffect(()=>{ (async()=>{
    const fabric = await getFabric(); fabricNs.current = fabric;
    ...
    fCanvas.current = c; (window as any).__bl_fcanvas = c;

    c.setWidth(PAGE_W); c.setHeight(PAGE_H); c.setZoom(zoom);
    ...
    if(!(c as any).__bl_eventsBound){
      ...
      c.on("selection:created",()=>{ if(syncingRef.current) return; select(computeSel()); });
      c.on("selection:updated",()=>{ if(syncingRef.current) return; select(computeSel()); });
      c.on("selection:cleared",()=>{ if(syncingRef.current) return; select([]); });

      c.on("object:moving",(e:any)=>{
        const o=e.target; if(!o||o.selectable===false) return;
        if(o.__bl_part) return; // Parts nicht verschieben
        o.set({ left:Math.round(o.left/snapSize)*snapSize, top:Math.round(o.top/snapSize)*snapSize });
      });

      c.on("object:scaling",(e:any)=>{
        const o=e.target; if(!o||o.selectable===false) return;
        if(o.__bl_part) return;
      ...
      c.on("object:modified",(e:any)=>{
        const o=e.target; if(!o||!o.__bl_id) return;
        if(o.__is_container) updateFrame(o.__bl_id,{ x:o.left, y:o.top, width:o.width, height:o.height });
      });

      c.on("mouse:dblclick",(e:any)=>{
        const t=e.target;
        if(t && t.type==="textbox" && typeof (t as any).enterEditing==="function"){
          (t as any).enterEditing(); (t as any).hiddenTextarea?.focus();
        }
      });
      ...
      const onDeleteActive=()=> {
        ...
        if(ids.length) useDesignerStore.getState().deleteByIds(ids);
  ...
  return ()=>{
    ...
    if(c?.__bl_onDeleteActive) window.removeEventListener("bl:delete-active", c.__bl_onDeleteActive);
    try{ c?.dispose?.(); }catch{}
    try{ const el=canvasRef.current as any; if(el && el.__fabricCanvas===c) el.__fabricCanvas=undefined; }catch{}
    fCanvas.current=null; fabricNs.current=null; objectsByKey.current.clear();
  };
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(()=>{
    const c=fCanvas.current; if(!c) return;
  ...
  useEffect(()=>{
    const c=fCanvas.current; if(!c) return;
    c.setZoom(zoom); c.setWidth(PAGE_W*zoom); c.setHeight(PAGE_H*zoom); c.requestRenderAll();
  },[zoom]);

  useEffect(()=>{
    const c=fCanvas.current; const fabric=fabricNs.current; if(!c||!fabric) return;
...
function ensureOverlay(c:any, fabric:any, margins:{top:number;right:number;bottom:number;left:number}){
  ...
  if(!objs.some(o=>o.__is_bg)){
    const bg=new fabric.Rect({ left:0, top:0, width:PAGE_W, height:PAGE_H, fill:"#ffffff", selectable:false, evented:false, hoverCursor:"default" });
    (bg as any).__is_bg=true; c.add(bg);
  }
  if(!objs.some(o=>o.__is_marginRect)){
    const marginRect=new fabric.Rect({
      ...
      width: PAGE_W - margins.left - margins.right,
      height: PAGE_H - margins.top - margins.bottom,
      fill:"rgba(0,0,0,0)", stroke:"#93c5fd", strokeDashArray:[6,6], selectable:false, evented:false, strokeUniform:true
    });
    (marginRect as any).__is_marginRect=true; c.add(marginRect);
  }
  c.requestRenderAll();
}
...
function purgeStrays(c:any){
  for(const o of [...(c.getObjects() as any[])]) if(!o.__bl_id && !o.__is_bg && !o.__is_marginRect) c.remove(o);
  c.requestRenderAll();
}
function charSpacingFromEm(em?:number){ if(em==null) return undefined; return Math.round(em*1000); }
function applyTextStyle(obj:any, base:{fontFamily?:string;fontSize?:number;lineHeight?:number;color?:string}, local?:PartStyle, global?:PartStyle){
  const s={...(base||{})}, g=global||{}, l=local||{};
  obj.set({
    fontFamily: l.fontFamily ?? g.fontFamily ?? s.fontFamily ?? "Inter, Arial, sans-serif",
    fontSize:   l.fontSize   ?? g.fontSize   ?? s.fontSize   ?? 12,
    lineHeight: l.lineHeight ?? g.lineHeight ?? s.lineHeight ?? 1.4,
    fill:       l.color      ?? g.color      ?? s.color      ?? "#111111",
    fontWeight: l.fontWeight ?? g.fontWeight ?? "normal",
    fontStyle:  (l.italic ?? g.italic) ? "italic" : "normal",
    charSpacing: charSpacingFromEm(l.letterSpacing ?? g.letterSpacing),
  });
}
function keyForGlobal(group:string, partKey:string){ return `${group}:${partKey}`; }

function reconcileCanvas(
  c:any, fabric:any, elements:CanvasElement[], tokens:any, partStyles:Record<string,PartStyle>, map:Map<string,any>
){
  purgeStrays(c); hydrateMapFromCanvas(c,map);
  ...
  for(const el of elements){
    if(el.kind==="photo"){
      ...
      let obj=map.get(key);
      if(!obj){
        obj=new fabric.Rect({ left:el.frame.x, top:el.frame.y, width:el.frame.width, height:el.frame.height, fill:"#e5e7eb", stroke:"#9ca3af", strokeUniform:true, selectable:true });
        obj.__bl_id=el.id; c.add(obj); map.set(key,obj);
      }
      obj.set({ left:el.frame.x, top:el.frame.y, width:el.frame.width, height:el.frame.height });
      keep.add(key); continue;
    }

    const secKey=`sec:${el.id}`;
    let container=map.get(secKey);
    if(!container){
      container=new fabric.Rect({ left:el.frame.x, top:el.frame.y, width:el.frame.width, height:el.frame.height, fill:"rgba(0,0,0,0)", stroke:"#e5e7eb", strokeUniform:true, selectable:true });
      container.__bl_id=el.id; container.__is_container=true; c.add(container); map.set(secKey,container);
    }else{
      container.set({ left:el.frame.x, top:el.frame.y, width:el.frame.width, height:el.frame.height });
    }
    keep.add(secKey);

    // *** falls alte persistierte Section ohne Parts: überspringen (kein leerer Text) ***
    if(!Array.isArray((el as any).parts) || !(el as any).parts.length) continue;

    for(const part of (el as any).parts){
      const pKey=`part:${el.id}:${part.key}`;
      let pObj=map.get(pKey);

      const base = { fontFamily:tokens?.fontFamily, fontSize:tokens?.fontSize, lineHeight:tokens?.lineHeight, color:tokens?.colorPrimary };
      ...
      if(!pObj){
        pObj=new fabric.Textbox(part.text || "", {
          ...
          hoverCursor:"text", moveCursor:"text",
        });
        pObj.__bl_id=el.id; pObj.__bl_part=part.key; pObj.__bl_group=el.group;
  ...
  for(const [key,obj] of Array.from(map.entries())){
    if(!keep.has(key)){ c.remove(obj); map.delete(key); }
  }
  c.requestRenderAll();
}
