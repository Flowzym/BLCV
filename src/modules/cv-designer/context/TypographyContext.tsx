// src/modules/cv-designer/context/TypographyContext.tsx
...
export const TypographyProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [typo, setTypoState] = useState<TypographyConfig>(defaultTypography);
  const setTypo = (p: Partial<TypographyConfig>) => setTypoState((t) => ({ ...t, ...p }));
  return <Ctx.Provider value={{ typo, setTypo }}>{children}</Ctx.Provider>;
};

export const useTypography = () => {
  const v = useContext(Ctx);
  if (!v) throw new Error('useTypography must be used within TypographyProvider');
  return v;
};
