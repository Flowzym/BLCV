...
// CVPreview wurde entfernt – wir nutzen eine leichte Inline-Preview:
import { LayoutElement } from '../modules/cv-designer/types/section';
import { StyleConfig } from '../types/cv-designer';

// -------- Inline Preview (ersetzt entferntes CVPreview) --------
const PAGE_W = 600;
const PAGE_H = 842;

function renderContent(el: LayoutElement, cv: any): string {
  switch (el.type) {
    case 'profil':
      return [
        `${cv.personalData.firstName || ''} ${cv.personalData.lastName || ''}`.trim(),
        cv.personalData.profession || '',
        [cv.personalData.email, cv.personalData.phone].filter(Boolean).join(' · '),
        cv.personalData.address || '',
        '',
    ...
    case 'erfahrung':
      return (cv.workExperience || [])
        .map((w: any) =>
          [
            `${w.position || ''} – ${w.company || ''} (${w.startDate || ''}–${w.endDate || ''})`.trim(),
    ...
    case 'ausbildung':
      return (cv.education || [])
        .map((e: any) =>
          [
            `${e.degree || ''} – ${e.institution || ''} (${e.startDate || ''}–${e.endDate || ''})`.trim(),
            e.fieldOfStudy ? `• ${e.fieldOfStudy}` : '',
            e.grade ? `• Abschluss: ${e.grade}` : ''
          ].filter(Boolean).join('\n')
        )
        .join('\n\n');
    case 'kenntnisse':
      return (cv.skills || [])
        ...
        .join('\n');
    case 'softskills':
      // Falls keine Softskills vorhanden, Platzhalter
      return ['• Teamfähigkeit', '• Kommunikationsstärke', '• Problemlösung'].join('\n');
    default:
      return '';
  }
}

const CVPreviewLite: React.FC<{
  layoutElements: LayoutElement[];
  cvData: any;
  scale?: number;
  showDebugBorders?: boolean;
}> = ({ layoutElements, cvData, scale = 1, showDebugBorders = false }) => {
  return (
    <div
      style={{
        width: PAGE_W,
      ...
      {layoutElements.map((el) => (
        <div
          ...
          style={{
            ...
            padding: 8,
            overflow: 'hidden',
            border: showDebugBorders ? '1px dashed #e5e7eb' : 'none',
            borderRadius: 6,
          }}
        >
          {el.title && <h4 className="text-sm font-semibold mb-1">{el.title}</h4>}
          <pre className="text-xs text-gray-700 whitespace-pre-wrap leading-5">
            {renderContent(el, cvData)}
          </pre>
        </div>
      ))}
    </div>
...
const mockCVData = {
  personalData: {
    ...
    summary:
      'Erfahrener Software Engineer mit Fokus auf moderne Webtechnologien und agile Entwicklungsmethoden. Spezialisiert auf React, TypeScript und Node.js Entwicklung.',
  },
  workExperience: [
    {
      id: 'exp-1',
          ...
          <div className="flex-1 bg-gray-50 overflow-y-auto">
            <div className="p-6">
              ...
              <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Current Typography Settings</h3>
                <div className="text-xs text-gray-600 space-y-1">
                  <div>
                    Selected: <span className="font-medium">{selectedSection}.{selectedField}</span>
                  </div>
    ...
    const PerformanceTestStory = () => {
      ...
      const runPerformanceTest = async () => {
        ...
        for (let i = 0; i < 50; i++) {
          setTimeout(() => {
            setUpdateCount(i + 1);
            // Hier würden echte Typography-Updates passieren
          }, i * 20);
        }
